class TimeSerieVars extends ZCustomController {
    ...
    async refresh() {
        if (!this.analyzer.watcher1) {
            let selector = GEOOSQuery.newEmptySelector(window.toLang("$[javascripts.timeserie_01]"), this.analyzer.layer.minZDimension, this.analyzer.layer.name);
        ...
        if (!this.analyzer.watcher2) {
            let selector = GEOOSQuery.newEmptySelector(window.toLang("$[javascripts.timeserie_02]"), this.analyzer.layer.minZDimension, this.analyzer.layer.name);
