...
def salvar_ultima_pasta_processada(nome_pasta):
    with open('/opt/geoos/NORTE/ultima_pasta_processada.txt', 'w') as f:
        f.write(nome_pasta)

# Função para ler o nome da última pasta processada
def ler_ultima_pasta_processada():
    try:
        with open('/opt/geoos/NORTE/ultima_pasta_processada.txt', 'r') as f:
            return f.read().strip()
...
ftp.cwd('/Public/BSO/Backup/AM_PA_3D')
...
if ultima_pasta != ultima_pasta_processada:
    ...
    for nome_arquivo in arquivos_para_baixar:
        caminho_local = f'/opt/geoos/data/UFPR/NORTE/{nome_arquivo}'
